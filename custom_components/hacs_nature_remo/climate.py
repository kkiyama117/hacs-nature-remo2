...
from remo import AirConParams
from remo.models import AirConRangeMode
from homeassistant.components.climate import ClimateEntity, HVACMode

from custom_components.hacs_nature_remo.domain.const import (DEFAULT_NAME, DOMAIN, ICON, SWITCH)

from . import HacsNatureRemoDataUpdateCoordinator, LOGGER, PluginDataDict
from .domain.climate import CLIMATE_MODE_REMO_TO_HA, CLIMATE_MODE_HA_TO_REMO
from .domain.config_schema import CLIMATE_CONFIG_SCHEMA, KEY_APPLIANCES, CONF_CLIMATE_KEY, KEY_DEVICES
...
async def async_setup_entry(hass, entry: ConfigEntry, async_add_devices):
    ...
    async_add_devices([
        ...
        for appliance_key, app in appliances.items() if app.type == "AC"
    ])
...
class HacsNatureRemoAC(HacsNatureRemoApplianceEntity, ClimateEntity):
    ...
    def __init__(self, coordinator: HacsNatureRemoDataUpdateCoordinator, idx, entry: ConfigEntry):
        ...
        self._inner_data: HacsNatureRemoACData = HacsNatureRemoACData(
            ...
            all_modes=self.appliance.aircon.range.get("modes"),
            current_fan=None,
            current_swing=None,
    ...
    def _update_data(self, ac_settings, device=None):
        self._update_inner_data(ac_settings, device)
        self._update_from_inner_data()

    # Fetch from coordinator
    def _update_inner_data(self, ac_settings: AirConParams, device: remo.Device | None = None):
        previous_data: HacsNatureRemoACData = deepcopy(self._inner_data)
        try:
            current_mode = CLIMATE_MODE_REMO_TO_HA.get(ac_settings.mode)
        except:
        ...
        try:
            ...
            _last_target_temperatures[_last_target_temperature_key] = _last_target_temperature_value
        except:
        ...
        new_one = HacsNatureRemoACData(
            ...
            current_fan=_fan, current_swing=_swing,
    ...
    def _update_from_inner_data(self):
        ...
        temp_range = self._inner_data.current_mode_temp_range
        if len(temp_range) >= 2:
        ...
        self._attr_fan_modes = self._inner_data.all_modes.get(
            CLIMATE_MODE_HA_TO_REMO.get(self._inner_data.current_mode)).vol
        self._attr_swing_mode = self._inner_data.current_swing
        self._attr_swing_modes = self._inner_data.all_modes.get(
            CLIMATE_MODE_HA_TO_REMO.get(self._inner_data.current_mode)).dir
    ...
    async def async_set_hvac_mode(self, hvac_mode: HVACMode) -> None:
        ...
        mode = CLIMATE_MODE_HA_TO_REMO[hvac_mode]
        if mode == CLIMATE_MODE_HA_TO_REMO[HVACMode.OFF]:
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        ap_data = aps_data.get(self.appliance.id).get("settings")
        device_data = self.coordinator.data.get(KEY_DEVICES).get(self.appliance.device.id)
    ...
    def _get_current_mode_temp_range(self, mode_name: HVACMode) -> list[float]:
        temp_range = self._inner_data.all_modes.get(CLIMATE_MODE_HA_TO_REMO.get(mode_name)).temp
        return list(map(float, filter(None, temp_range)))

    @staticmethod
    def _convert_to_hvac_list(modes: dict[str, remo.AirConRangeMode] | None) -> list[HVACMode]:
        remo_modes = list(modes.keys())
        ha_modes = list(map(lambda mode: CLIMATE_MODE_REMO_TO_HA.get(mode), remo_modes))
